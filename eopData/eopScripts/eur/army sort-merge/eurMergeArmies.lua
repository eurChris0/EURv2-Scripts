...
local function getCurrentTurn()
    if to_log then
		M2TWEOP.logGame("EUR SCRIPT: "..tostring(os.clock()).."getCurrentTurn");
	end
...
local function distanceFromPoint(army, targetArmy)
    if to_log then
		M2TWEOP.logGame("EUR SCRIPT: "..tostring(os.clock()).."distanceFromPoint");
	end
...
local function mergeLogPrint(targetStack, mergeType)
    if to_log then
		M2TWEOP.logGame("EUR SCRIPT: "..tostring(os.clock()).."mergeLogPrint");
	end
...
local function mergeAiArmies(targetArmy)
    if to_log then
		M2TWEOP.logGame("EUR SCRIPT: "..tostring(os.clock()).."mergeAiArmies");
	end
...
function MergingArmies.mergeFactionArmies(faction)
    if to_log then
		M2TWEOP.logGame("EUR SCRIPT: "..tostring(os.clock()).."mergeFactionArmies");
	end
    ...
    if (currentTurn % 20 == 0) then return false end
